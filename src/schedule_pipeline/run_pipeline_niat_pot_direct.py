#!/usr/bin/env python3
"""
Enhanced NIAT PoT Pipeline with Iterative Retry and Direct Answer
...
from utils.evaluator import niat_match_func_for_samples
...
def parse_args():
    parser = argparse.ArgumentParser(description="Enhanced NIAT PoT Pipeline with Direct Answer")
    ...
    parser.add_argument('--tmp_save_path', type=str,
                       default='datasets/schedule_test/niat_pot_direct',
...
def load_niat_dataset(json_path: str, first_n: int = -1, random_sample: bool = False) -> List[Dict]:
...
def build_pot_prompt_with_error(item: Dict, df: pd.DataFrame, template_path: str,
    ...
    # Convert DataFrame to dict format for prompt
    table_dict = {
...
def main():
    ...
    print("="*80)
    print("NIAT PoT Pipeline - Direct Answer Version")
    print("="*80)
    print(f"Timestamp: {timestamp}")
    print(f"Save Path: {args.tmp_save_path}")
    print(f"Dataset: {args.niat_json_path}")
    ...
    raw_data = load_niat_dataset(args.niat_json_path, args.first_n, args.random_sample)
    ...
    if need_llm_indices:
        ...
        qa_template_path = os.path.join(os.path.dirname(__file__), '../prompts/text_reason_niat.txt')
        ...
        for idx in need_llm_indices:
            ...
            table_title = item.get('table_title', item.get('table_id', 'Table'))
...
<output>"""
    ...
    # Build samples for NIAT evaluation
    all_samples = []
    for i in range(len(raw_data)):
        sample = {
            'chain': [{'parameter_and_conf': [(preds[i] if preds[i] else '', 1.0)]}],
            'answer': raw_data[i].get('answer', '')
        }
    ...
    results_df = pd.DataFrame({
        ...
        'table_id': [item.get('table_id', '') for item in raw_data],
        'question': [item.get('question', '') for item in raw_data],
        'gold_answer': [item.get('answer', '') for item in raw_data],
        'prediction': preds,
    ...
    preds_and_golds = []
    for i in range(len(raw_data)):
        preds_and_golds.append({
            'id': raw_data[i].get('table_id', ''),
            'prediction': preds[i] if preds[i] else '',
            'answer': raw_data[i].get('answer', '')
        })
    
    with open(os.path.join(args.tmp_save_path, 'preds_and_golds.json'), 'w') as f:
        json.dump(preds_and_golds, f, indent=2)
