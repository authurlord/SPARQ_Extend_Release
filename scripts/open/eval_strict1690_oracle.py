...
def _bm25_rank(query: str, candidates: list[tuple[str, str]]) -> list[int]:
    """BM25 rank the (anchor, text) candidates by query relevance, return indices sorted best-first."""
...
def render_gold_table(t: dict, pool: dict[str, str], question: str,
                      max_linked: int = 30, max_rows: int = 50,
                      passage_chars: int = 2500, cell_chars: int = 200
                      ) -> tuple[str, list[str]]:
    """SPARQ-style table render + BM25-ranked top-max_linked linked passages."""
    ...
    # First pass: collect ALL (anchor, text) candidates from the gold table
    ...
    # BM25 rank candidates by query relevance, keep top-max_linked
    ...
    async def _one(q):
        ...
        async with sem:
            ...
            if not t:
                ...
                table_text, passages = render_gold_table(t, pool, q["question"])
